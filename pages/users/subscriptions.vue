<template>
  <div>
    <SingleThread v-for="thread in subscriptions" :key="thread.id" :thread="thread"></SingleThread>
    <Pagination :pagination="subscriptions_paginate" routeName="profile.show.subscriptions" :param="{key:'username',value:''}" />
  </div>
</template>
    ...
    computed:{
     ...
     async fetch({ params, query, error, $axios, store,redirect }) {
      try {
        ...
        .map(k => `${k}=${query[k]}`)
        .join('&');
        const subscriptionsRresponse = await $axios.$get(`profile/${params.username}/subscriptions?${q}`);
