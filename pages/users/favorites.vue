<template>
  <div>
    <SingleThread v-for="thread in favorites" :key="thread.id" :thread="thread"></SingleThread>
    <Pagination :pagination="favorites_paginate" routeName="profile.show.favorites" :param="{key:'username',value:''}" />
  </div>
</template>
    ...
    computed:{
     ...
     async fetch({ params, query, error, $axios, store,redirect }) {
      try {
        ...
        .map(k => `${k}=${query[k]}`)
        .join('&');
        const favoriteRresponse = await $axios.$get(`profile/${params.username}/favorites?${q}`);
